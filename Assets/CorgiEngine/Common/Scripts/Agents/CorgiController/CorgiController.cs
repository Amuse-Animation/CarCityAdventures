...
namespace MoreMountains.CorgiEngine
{
    ...
    public class CorgiController : CorgiMonoBehaviour
    {
        ...
        {
            //if (float.IsNaN(_newPosition.x) || float.IsNaN(_newPosition.y)) return;
            if (float.IsNaN(_newPosition.x) && !float.IsNaN(_newPosition.y))
            {
                _newPosition = Vector2.up * _newPosition.y;
            }

            else if (!float.IsNaN(_newPosition.x) && float.IsNaN(_newPosition.y))
            {
                _newPosition = Vector2.right * _newPosition.x;
            }

            else if ((float.IsNaN(_newPosition.x) || float.IsNaN(_newPosition.y))) return;
        ...
        public void SetDefaultParameterGravityValue(float gravityValue)
        {
            DefaultParameters.Gravity = gravityValue;
        }

        public void SetDefaultParameterFallMultiplier(float multiplier)
        {
            DefaultParameters.FallMultiplier = multiplier;
        }

        public void SetDefaultParameterAscentMultiplier(float multiplier)
        {
            DefaultParameters.AscentMultiplier = multiplier;
        }

        public void SetDefaultParameterMaxSpeed(Vector2 newSpeed)
        {
            DefaultParameters.MaxVelocity = newSpeed;
        }

        public void SetDefaultParameter(CorgiControllerParameters dfParameter)
        {
            if (dfParameter == null) return;

            SetDefaultParameterGravityValue(dfParameter.Gravity);
            SetDefaultParameterAscentMultiplier(dfParameter.AscentMultiplier);
            SetDefaultParameterFallMultiplier(dfParameter.FallMultiplier);
            SetDefaultParameterMaxSpeed(dfParameter.MaxVelocity);
        }
    }
}